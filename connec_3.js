...
app.get("/", function(req, res) {
    ...
    con.query(sql, function(err, results) {
        if (err) throw err;
        res.render("connec_3.ejs", { result: results })
        console.log(results)
    });
...
app.post("/fix-todo", function(req, res) {
    console.log("Đã nhận request", req.body)
    var sql = `UPDATE video Set link ='${req.body.newlink}', img='${req.body.newimg}', title = '${req.body.newtitle}', channel = '${req.body.newchannel}', view = '${req.body.newview}',time ='${req.body.newtime}' WHERE title='${req.body.title}'`
    con.query(sql, function(err, result) {
        if (err) throw err;
    });
})
...
app.listen(3001, function() {
    console.log("Hello nodejs running on port 3001")
})